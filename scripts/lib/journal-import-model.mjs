...
export function validateJournalSource(source, blogBodies) {
  ...
  for (const design of source.gameDesigns) {
    if (!DIGEST_PATTERN.test(design.sha256 ?? '')) throw new Error(`Invalid game design catalog digest for ${design.id}.`);
    if (typeof design.title !== 'string' || !design.title.trim()) throw new Error(`Missing game design title for ${design.id}.`);
    if (typeof design.summary !== 'string' || !design.summary.trim()) throw new Error(`Missing game design summary for ${design.id}.`);
    if (!Array.isArray(design.tags) || design.tags.length === 0) throw new Error(`Missing game design tags for ${design.id}.`);
  }
}
