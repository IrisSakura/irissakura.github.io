...
export function renderLivingHome({ projects, site, presentations, presentation, now, updates, articles, series, brand, personas }) {
  ...
  const sections = {
    ...
    writing: `<section id="writing" class="home-writing living-section"><div class="container">${heading('THINK / MYOSOTIS', '最近写的', link('/pages/blog.html', '全部文章 →', ''))}<div class="writing-list">${writing.map((article) => `<article class="writing-entry" data-article-id="${escape(article.id)}"><time datetime="${article.publishedAt}">${article.publishedAt}</time><h3>${link(`/pages/blog/${article.slug}.html`, article.title, '')}</h3><p>${escape(article.summary)}</p></article>`).join('')}</div></div></section>`,
    mods: `<section id="mods" class="home-mod-series" data-brand-layout="editorial" aria-labelledby="home-mod-series-title"><div class="container home-mod-series-inner"><img src="${escape(brand.assets.freesiaLogoSmall)}" alt="" width="112" height="112" loading="lazy" decoding="async"><div><p class="section-kicker">EXPLORE</p><h2 id="home-mod-series-title">${escape(series.displayName)}</h2><p>我为喜欢的游戏做的一些 Mod。</p></div>${link('/pages/mods.html', '浏览 Mods', '', 'btn btn-secondary')}</div></section>`,
    projects: `<section id="projects" class="home-build living-section"><div class="container">${heading('BUILD', '把创作需要的东西，一起做出来', link('/pages/development.html','查看项目关系 →',''))}<div class="ecosystem-bridge">${presentations.filter(p=>['iris-engineering','sakura-framework','iris-shelf'].includes(p.projectId)).map((p,i)=>`<article data-project-id="${p.projectId}" data-persona="${p.personaId}"><span>0${i+1}</span><img src="${brand.assets[p.logoAssetKey]}" alt="" width="48" height="48"><h3>${link(p.route,p.displayName,'')}</h3><p>${esc(p.summary)}</p></article>`).join('')}</div></div></section>`,
    live: `<section class="home-live living-section" id="live"><div class="container"><p class="section-kicker">LIVE / WISTERIA</p><h2>也给生活留下一小片空间。</h2><p>我在探索一个持续存在的桌面小世界，让工作之外的时间也有可以停留的地方。</p>${link('/pages/wisteria.html','走近 Wisteria →','')}</div></section>`,
