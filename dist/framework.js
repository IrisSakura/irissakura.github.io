...
const FALLBACK_MODULES = Object.entries(MODULE_PRESENTATIONS).map(([id, presentation]) => ({
    id,
    displayName: {
        core: 'Core 核心',
        event: 'Event 事件',
...
class FrameworkPage {
    constructor() {
        ...
        this.moduleQuery = '';
        this.selectedModuleId = 'core';
        this.selectedLayerId = 'foundation';
        this.selectedLifecycle = 'Supported';
    ...
    selectModule(moduleId, updateHash = true) {
    ...
    renderLayers() {
        ...
        if (!layerList || this.layers.length === 0)
            return;
    ...
    renderLifecycle() {
        ...
        if (!lifecycleList || Object.keys(this.lifecycleCounts).length === 0)
            return;
        ...
        if (!names.includes(this.selectedLifecycle))
            this.selectedLifecycle = names[0];
    ...
    selectLifecycle(name) {
        ...
        document.querySelectorAll('[data-lifecycle-name]').forEach(button => {
    ...
    restoreSectionHash() {
        ...
        if (!['modules', 'architecture', 'lifecycle'].includes(targetId))
            return;
    ...
    setupEventListeners() {
        ...
        document.querySelectorAll('[data-module-filter]').forEach(button => {
            button.addEventListener('click', () => {
                ...
                document.querySelectorAll('[data-module-filter]').forEach(candidate => {
        ...
        document.getElementById('framework-module-list')?.addEventListener('click', event => {
            ...
            if (button?.dataset.moduleId)
                this.selectModule(button.dataset.moduleId);
        });
        document.getElementById('framework-layer-list')?.addEventListener('click', event => {
            const target = event.target;
            if (!(target instanceof Element))
                return;
            const button = target.closest('[data-layer-id]');
            if (button?.dataset.layerId)
                this.selectLayer(button.dataset.layerId);
        });
