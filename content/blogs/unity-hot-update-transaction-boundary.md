...
**热更新事务提交边界（后文简称“整套换挡点”）**：新版本的存储指向与运行时状态都已完成切换，失败恢复责任也已经结清的边界。
...
**不可变版本目录（后文简称“候选仓”）**：每个已验证版本写入独立目录，已存版本不在原地改写。

**活动版本指针（后文简称“指路牌”）**：一个很小的持久化记录，指出下一次启动应采用哪个版本及其包指纹。

**更新 Journal（后文简称“进度单”）**：记录一次更新的操作身份、旧指针、候选指针与最近完成阶段。
...
**Shadow Runtime（后文简称“后台试运行”）**：候选内容已经被解析、编译或组装，但尚未发布为全局可见状态的运行时。
...
我更愿意把它称为**失败封闭（后文简称“看不懂就别继续动”）**：当系统无法确认真实状态时，停止继续更新，把问题暴露给诊断和人工处置。
