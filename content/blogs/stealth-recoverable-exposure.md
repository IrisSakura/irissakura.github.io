...
**可恢复暴露循环（后文简称“被发现了也还有救”）**：玩家暴露后，警戒和敌人行动发生升级，但玩家仍能通过脱离、误导、切断传播或改变身份回到可继续行动的状态。
...
**感知不对称（后文简称“双方看到的不是同一份真相”）**：玩家、守卫和安防设备只能从各自位置与能力出发，获得局部且可能过时的信息。
...
**世界真实状态（后文简称“事情实际上怎样”）**：游戏世界中已经发生、但不一定被任何角色知道的完整事实。

**敌人认知状态（后文简称“守卫以为事情怎样”）**：某个敌人根据感知、记忆、证据和他人报告建立的局部判断。
...
**发现进度（后文简称“逐渐看清”）**：敌人对一个刺激进行识别和确认时，随时间积累并可因条件变化而衰减的认知量。
...
**警戒状态机（后文简称“从起疑到封锁的台阶”）**：用一组有明确进入条件、行为变化和退出条件的阶段，描述敌方警戒如何演化。
...
**最后已知位置（后文简称“守卫最后确定你在哪”）**：敌人在某一时刻通过可靠感知确认的目标位置，以及该信息产生的时间和置信度。
...
**信息传播延迟（后文简称“一个守卫知道，不等于全地图都知道”）**：信息从发现者传递给其他角色或系统时，由报告动作、通信网络、设备状态和距离产生的时间与范围限制。
...
**证据持久性（后文简称“留下的痕迹不会自己消失”）**：尸体、血迹、失窃物、破坏设备、门禁日志等事实在警戒下降后仍能继续影响世界。
...
**新常态（后文简称“警报降了，但世界没失忆”）**：高警戒结束后，世界恢复为可继续行动的稳定状态，同时保留由先前暴露造成的部分改变。
