...
test('synchronized source dates may advance without rewriting reviewed project facts', async () => {
  const [projects, framework, journal, publication] = await Promise.all([
    readJson('data/projects.json'),
    readJson('data/framework.json'),
    readJson('data/journal.json'),
    readJson('config/blog-publication.json')
  ]);
  assert.doesNotThrow(() => assertProjectFactsCurrent(projects, framework, journal, publication));
  ...
  assert.doesNotThrow(
    () => assertProjectFactsCurrent(projects, nextFrameworkSnapshot, nextJournalSnapshot, publication)
  );
});
...
test('project facts still reject updates newer than their review date', async () => {
  ...
  assert.throws(
    () => assertProjectFactsCurrent(unreviewed, framework, journal, publication),
...
test('semantic Framework and Journal contracts require project copy review', async () => {
  ...
  assert.equal(
    ...
    journalCurationReviewHash(journal, publication)
  );

  const changedFramework = { ...framework, adoptionReviewHash: `sha256:${'f'.repeat(64)}` };
  assert.throws(
    () => assertProjectFactsCurrent(projects, changedFramework, journal, publication),
  ...
  assert.throws(
    () => assertProjectFactsCurrent(projects, framework, changedJournal, publication),
