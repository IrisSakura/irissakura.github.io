...
test('series, tags and related articles form indexable discovery routes', async () => {
  ...
  for (const collection of [...discovery.series, ...discovery.tags]) {
  ...
  for (const article of articles) {
    ...
    for (const tag of article.tags) {
      assert.ok(html.includes(`tag/${discovery.tagsByName.get(tag).slug}.html`));
    }
  }
});
